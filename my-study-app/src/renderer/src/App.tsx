...
declare global {
  interface Window {
    api: {
      saveLog: (data: any) => Promise<{ success: boolean; path?: string; error?: string }>
    }
  }
}
...
const handleSave = async () => { // async をつけるのを忘れるな
    const logData = {
    ...
    <div style={{ padding: '20px', maxWidth: '600px', margin: '0 auto', fontFamily: 'sans-serif', color: '#333' }}>
      ...
      {mode === 'idle' && (
        <div style={{ textAlign: 'center', marginTop: '50px' }}>
          ...
          <button 
            onClick={handleStart}
            style={{ padding: '15px 40px', fontSize: '20px', backgroundColor: '#28a745', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer' }}
          >
            COMMIT & START
          </button>
        </div>
      ...
      {mode === 'running' && (
        <div style={{ textAlign: 'center', marginTop: '50px' }}>
          <h2>Focusing...</h2>
          <div style={{ fontSize: '100px', fontWeight: 'bold', fontFamily: 'monospace', margin: '20px 0' }}>
            {formatTime(timeLeft)}
          </div>
          <button 
      ...
      {/* 3. 振り返り画面 (今回の核心) */}
      {mode === 'review' && (
        <div style={{ backgroundColor: '#f8f9fa', padding: '20px', borderRadius: '10px', border: '1px solid #ddd' }}>
          <h2 style={{ color: '#0056b3' }}>Session Complete.</h2>
          <p>お疲れ様でした．脳内のキャッシュを書き出してください．</p>
          
          <div style={{ marginBottom: '15px' }}>
            <label style={{ display: 'block', fontWeight: 'bold', marginBottom: '5px' }}>Acquisition (理解したこと・成果)</label>
            <textarea 
              value={acquisition}
          ...
          <div style={{ marginBottom: '15px' }}>
            <label style={{ display: 'block', fontWeight: 'bold', marginBottom: '5px' }}>Debt (理解できなかったこと・負債)</label>
            <textarea 
              value={debt}
          ...
          <div style={{ marginBottom: '20px' }}>
            <label style={{ display: 'block', fontWeight: 'bold', marginBottom: '5px' }}>Next Action (次の一手)</label>
            <input 
              type="text"
              value={nextAction}
          ...
          <button 
            onClick={handleSave}
            style={{ width: '100%', padding: '15px', fontSize: '18px', backgroundColor: '#007bff', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold' }}
          >
            SAVE LOG to CONSOLE
          </button>
        </div>
